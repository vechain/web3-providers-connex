...
describe('Test scenarios when errors are generated', function () {
	...
	it('invalid revision', async () => {
		...
		for (let i = 0; i < methods.length; i++) {
			try {
				await provider.request({ method: methods[i], params: params[i] })
				assert.fail('Test failed')
			} catch (err: any) {
				expect(err.message).to.eql(thor.ErrMsg.ArgumentMissingOrInvalid(methods[i], 'revision'))
			}
		}
	})

	it('test errors generated by Restful functions', async () => {
		const methods = [
			'eth_getCode',
			'eth_getStorageAt',
			'eth_getBalance',
		...
		for (let i = 0; i < methods.length; i++) {
			try {
				...
				expect(err instanceof ProviderRpcError).to.be.true
				expect((err as ProviderRpcError).stack?.includes('restful')).to.be.true
			}
		}
	})
})